...
from pulp_chart.app.serializers import ChartContentSerializer
from pulp_chart.app.models import ChartContent

from pulpcore.plugin.models import Artifact


# Fill data with sufficient information to create ChartContent
# Provide sufficient parameters to create the ChartContent object
# Depending on the base class of the serializer, provide either "_artifact" or "_artifacts"
@unittest.skip("FIXME: plugin writer action required")
class TestChartContentSerializer(TestCase):
    """Test ChartContentSerializer."""

    def setUp(self):
        """Set up the ChartContentSerializer tests."""
    ...
    def test_valid_data(self):
        """Test that the ChartContentSerializer accepts valid data."""
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = ChartContentSerializer(data=data)
        self.assertTrue(serializer.is_valid())

    def test_duplicate_data(self):
        """Test that the ChartContentSerializer does not accept data."""
        ChartContent.objects.create(artifact=self.artifact)
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = ChartContentSerializer(data=data)
