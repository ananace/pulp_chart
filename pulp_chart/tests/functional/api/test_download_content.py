...
from pulp_chart.tests.functional.constants import CHART_FIXTURE_URL
from pulp_chart.tests.functional.utils import (
    gen_chart_client,
    get_chart_content_paths,
    gen_chart_remote,
    monitor_task,
)
...
# Implement tests.functional.utils.get_chart_content_unit_paths(),
...
class DownloadContentTestCase(unittest.TestCase):
    ...
    def test_all(self):
        ...
        * `Pulp Smash #872 <https://github.com/pulp/pulp-smash/issues/872>`_
        ...
        distributions = DistributionsChartApi(client)
        ...
        unit_paths = [choice(paths) for paths in get_chart_content_paths(repo.to_dict()).values()]
        fixtures_hashes = [
            hashlib.sha256(utils.http_get(urljoin(CHART_FIXTURE_URL, unit_path[0]))).hexdigest()
